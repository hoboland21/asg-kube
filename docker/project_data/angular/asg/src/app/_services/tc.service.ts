...
export class TcService {
  ...
  public postTCmd(cmd:IAction): Observable<any> {
    return  this.getSession().pipe(
      tap((auth) => { cmd.username = auth.user;
          cmd.time =  new Date().toLocaleString('en-US', { timeZone: 'PST' })
        }
      ),
      concatMap( () => this.http.post<any>(`${this.API}/cmd/`,cmd,httpOptions)),
      concatMap( () =>this.loggingService.postLog(cmd))
    )
  }
